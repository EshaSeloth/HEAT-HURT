...
def load_data():
    """
    Generates synthetic hair damage data that mirrors the MySQL database schema:
    - users table: user_id, age, hair_type
    - heat_usage table: usage_id, user_id, tool_type, temperature_range, duration_min, usage_freq, total_usage_months
    - hair_condition table: condition_id, user_id, damage_score
    Data is generated with realistic correlations (higher temp + daily use = higher damage).
    Original data sourced from a local MySQL database (heathurt).
    """
    np.random.seed(42)
    n = 10500
    ...
    users = pd.DataFrame({
        ...
        'hair_type': np.random.randint(1, 5, n),   # 1=straight,2=wavy,3=curly,4=coily
    })

    # Simulate heat_usage table
    heat = pd.DataFrame({
        'user_id':            range(1, n + 1),
        'usage_id':           range(1, n + 1),
        'tool_type':          np.random.randint(1, 4, n),     # 1=dryer,2=curler,3=straightener
        'temperature_range':  np.random.randint(100, 231, n),
        'duration_min':       np.random.randint(5, 76, n),
        'usage_freq':         np.random.randint(1, 4, n),     # 1=monthly,2=weekly,3=daily
        'total_usage_months': np.random.randint(1, 61, n),
    })

    # Simulate hair_condition table — damage score driven by real-world logic
    damage_score = (
        ((heat['temperature_range'] - 100) / 130 * 2) +
        (heat['duration_min'] / 75 * 1.5) +
        (heat['usage_freq'] * 0.5) +
        (heat['total_usage_months'] / 60 * 1) +
        ((users['hair_type'] - 1) / 3 * 0.5) +
        np.random.normal(0, 0.3, n)
    )
    damage_score = damage_score.clip(0, 5).round().astype(int).clip(0, 5)

    hair = pd.DataFrame({
        'user_id':      range(1, n + 1),
        'condition_id': range(1, n + 1),
        'damage_score': damage_score,
    })

    # Merge all three tables (mirrors MySQL join)
